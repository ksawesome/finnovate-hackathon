...
class FileFingerprinter:
    ...
    def check_duplicate(fingerprint: str) -> bool:
        ...
        existing = collection.find_one(
            {"event_type": "file_ingested", "file_fingerprint": fingerprint}
        )
...
class IngestionOrchestrator:
    ...
    def ingest_file(
        ...
        logger.log_event("ingestion_started", file=file_path, entity=entity, period=period)
        ...
        try:
            ...
            if validate_before_insert:
                ...
                try:
                    ...
                    v_result = orchestrator.validate_dataframe(
                        df, entity=entity, period=period, fail_on_critical=False
                    )
            ...
            if skip_duplicates and self.fingerprinter.check_duplicate(fingerprint):
                ...
                return {"status": "skipped", "reason": "duplicate", "fingerprint": fingerprint}
            ...
            return {
                ...
                "failed": result["failed"],
                "duration_seconds": (datetime.utcnow() - start_time).total_seconds(),
                **(validation_metrics if validation_metrics else {}),
            }

        except Exception as e:
            logger.log_event("error_occurred", level="ERROR", error=str(e))

            return {"status": "failed", "entity": entity, "period": period, "error": str(e)}

...
class GLAccount(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "account_code", "company_code", "period", name="uq_gl_account_company_period"
        ),
...
class ResponsibilityMatrix(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "gl_code", "company_code", "assignment_id", name="uq_assignment_gl_company"
        ),
...
def get_gl_accounts_by_period(period: str, company_code: str | None = None) -> list[GLAccount]:
...
def get_gl_account_by_code(account_code: str, company_code: str, period: str) -> GLAccount | None:
...
def update_gl_account(gl_account_id: int, **kwargs) -> GLAccount | None:
...
def create_responsibility_assignment(
    gl_code: str, company_code: str, assigned_user_id: int, **kwargs
) -> ResponsibilityMatrix:
    """Create a new responsibility assignment."""
    session = get_postgres_session()
    try:
        assignment = ResponsibilityMatrix(
            gl_code=gl_code, company_code=company_code, assigned_user_id=assigned_user_id, **kwargs
        )
...
def get_user_assignments(
    user_id: int, status_filter: str | None = None
) -> list[ResponsibilityMatrix]:
...
def create_master_account(
    account_code: str, account_description: str, **kwargs
) -> MasterChartOfAccounts:
    """Create a master chart of accounts entry."""
    session = get_postgres_session()
    try:
        master_account = MasterChartOfAccounts(
            account_code=account_code, account_description=account_description, **kwargs
        )
...
def get_master_account(account_code: str) -> MasterChartOfAccounts | None:
    """Get master account by code."""
    session = get_postgres_session()
    try:
        return (
            session.query(MasterChartOfAccounts)
            .filter(MasterChartOfAccounts.account_code == account_code)
            .first()
        )
    finally:
        session.close()
...
def get_account_hierarchy_children(parent_code: str) -> list[MasterChartOfAccounts]:
    ...
    try:
        return (
            session.query(MasterChartOfAccounts)
            .filter(MasterChartOfAccounts.group_gl_account == parent_code)
            .all()
        )
    finally:
        session.close()
...
def create_version_snapshot(
    gl_account_id: int, account_code: str, snapshot_data: dict, created_by: int, change_reason: str
) -> GLAccountVersion:
    ...
    try:
        ...
        latest = (
            session.query(GLAccountVersion)
            .filter(GLAccountVersion.gl_account_id == gl_account_id)
            .order_by(GLAccountVersion.version_number.desc())
            .first()
        )
...
def create_account_template(account_code: str, **kwargs) -> AccountMasterTemplate:
    ...
    session = get_postgres_session()
    try:
        template = AccountMasterTemplate(account_code=account_code, **kwargs)
...
def get_template_by_nature(nature: str) -> list[AccountMasterTemplate]:
    ...
    try:
        return (
            session.query(AccountMasterTemplate)
            .filter(AccountMasterTemplate.nature.ilike(f"%{nature}%"))
            .all()
        )
    finally:
        session.close()
...
def get_active_templates(department: str | None = None) -> list[AccountMasterTemplate]:
    """Get all active account templates."""
    session = get_postgres_session()
    try:
        query = session.query(AccountMasterTemplate).filter(AccountMasterTemplate.is_active is True)
        if department:
...
def log_review(gl_account_id: int, reviewer_id: int, status: str, comments: str = "") -> ReviewLog:
    ...
    try:
        review = ReviewLog(
            gl_account_id=gl_account_id, reviewer_id=reviewer_id, status=status, comments=comments
        )
        session.add(review)
        session.commit()
...
def get_account_review_history(gl_account_id: int) -> list[ReviewLog]:
    ...
    try:
        return (
            ...
            .filter(ReviewLog.gl_account_id == gl_account_id)

...
from src.reports import REPORT_TYPES, BaseReport, generate_report, list_available_reports
...
def test_generate_report_invalid_type(sample_entity, sample_period, test_output_dir):
    ...
    with pytest.raises(ValueError, match="Unknown report type"):
        generate_report('invalid_report_type', sample_entity, sample_period, output_dir=test_output_dir)
...
def test_status_report_generation(
    mock_get_accounts, mock_status_summary, mock_pending_items, mock_session,
    sample_entity, sample_period, test_output_dir
):
    """Test GL Account Status Report generation."""
    # Mock data
    mock_pending_items.return_value = {
        'pending_reviews': [
            {'account_code': 'GL001', 'account_name': 'Cash', 'criticality': 'High', 'department': 'Finance'}
        ],
        'missing_docs': [
            {'account_code': 'GL002', 'account_name': 'Inventory', 'criticality': 'Medium', 'closing_balance': 50000}
        ],
        'flagged_items': [
            {'account_code': 'GL003', 'account_name': 'Revenue', 'reason': 'Unusual variance', 'priority': 'High'}
        ]
    }
...
def test_variance_report_generation(
    mock_perform_analytics, mock_variance_analysis,
    sample_entity, sample_period, test_output_dir
):
...
def test_hygiene_report_generation(
    mock_perform_analytics, mock_hygiene_score,
    sample_entity, sample_period, test_output_dir, mock_hygiene_score as hygiene_data
):
    """Test GL Hygiene Dashboard Report generation."""
    # Mock data
    mock_hygiene_score.return_value = hygiene_data
    mock_perform_analytics.return_value = {
        'total_balance': 1000000,
        'account_count': 100
    }
...
def test_reviewer_performance_report(
    mock_get_accounts, mock_get_assignments, mock_session,
    sample_entity, sample_period, test_output_dir
):
    ...
    # Generate report
    result = generate_report('reviewer_performance', sample_entity, sample_period, output_dir=test_output_dir)
...
def test_sla_compliance_report(
    mock_get_assignments, mock_get_accounts, mock_session,
    sample_entity, sample_period, test_output_dir
):
    ...
    # Generate report
    result = generate_report('sla_compliance', sample_entity, sample_period, output_dir=test_output_dir)
...
def test_executive_summary_report(
    mock_review_status, mock_hygiene, mock_analytics, mock_insights, mock_exec_summary,
    sample_entity, sample_period, test_output_dir, mock_analytics_data, mock_hygiene_score
):
    ...
    # Generate report
    result = generate_report('executive_summary', sample_entity, sample_period, output_dir=test_output_dir)
...
def test_all_reports_generate_successfully(sample_entity, sample_period, test_output_dir):
    ...
    for report_type in report_types:
        try:
            result = generate_report(report_type, sample_entity, sample_period, output_dir=test_output_dir)
            assert result is not None
        except Exception as e:
            pytest.fail(f"Failed to generate {report_type} report: {str(e)}")
